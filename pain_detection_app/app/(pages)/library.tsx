...
export default function LibraryScreen() {
  ...
  const handleVideoReady = (status) => {
    if (
      ...
      Number.isFinite(status.naturalSize.width / status.naturalSize.height)
    ) {
      ...
      <View style={styles.container}>
        <View style={styles.videoAndVerdictContainer}>
          <View
            style={[
              styles.videoFrame,
              { borderColor: currentPain?.color || tintColor, aspectRatio: videoAspect },
            ]}
          >
            <Video
              ref={videoRef}
        ...
        <TouchableOpacity
          ...
          onPress={async () => {
            ...
            setVerdict(null);
            setVideoAspect(1);
            router.replace('/(pages)/'); // Return to main menu
          }}
        >
